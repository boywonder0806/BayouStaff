...
function nextMonday() {
  ...
  return format(addWeeks(currentMon, 1), 'yyyy-MM-dd');
}
...
export default function PlanSchedule() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const res = await api.get(`/admin/scheduler/plan?weekStart=${weekStart}`);
      setData(res.data);
  ...
  async function handleDrop(e, empId, date) {
    ...
    try {
      await api.patch(`/admin/scheduler/plan/shifts/${dragging.id}`, { employeeId: empId, date });
      load();
  ...
  async function handlePaste(empId, date) {
    ...
    try {
      if (mode === 'cut') {
        await api.patch(`/admin/scheduler/plan/shifts/${shift.id}`, { employeeId: empId, date });
        setClipboard(null);
      } else {
        const { id: _id, ...fields } = shift;
        await api.post('/admin/scheduler/plan/shifts', { ...fields, employeeId: String(empId), date });
      }
  ...
  const employees = useMemo(() => {
    ...
    if (sortBy === 'hours') return [...raw].sort((a, b) => draftHours(b.id) - draftHours(a.id));
  ...
  }, [data?.employees, deptFilter, sortBy, shifts, draftHours]);
    ...
    <div className="flex" style={{ height: 'calc(100vh - 3rem)' }}>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <div className="flex items-center justify-between mb-4 shrink-0">
          <div>
            <p className="label-xs mb-0.5">Plan Schedule</p>
          ...
          <div className="flex items-center gap-2">
            {clipboard && (
              <span className="text-xs text-fog border border-amber-500/30 rounded px-2 py-1 flex items-center gap-1.5">
            ...
            >
              {publishing ? 'Publishing…' : `Publish${shifts.length ? ` (${shifts.length})` : ''}`}
            </button>
          </div>
        </div>
                  ...
                  <th className="sticky left-0 top-0 z-20 bg-deep border-b border-r border-rim/40 px-4 py-2 text-left w-44">
                    ...
                    <div className="flex items-center gap-1 mt-1">
                      ...
                      <select
                        ...
                        className="text-10 bg-transparent text-amber-400 border-0 outline-none cursor-pointer font-bold"
                      >
                  ...
                  return (
                    <tr key={emp.id} className={ri % 2 === 0 ? '' : 'bg-shell/20'}>
                      ...
                      <td className="border-b border-r border-rim/40 text-center px-2 py-3">
                        <span className={`text-sm font-semibold tabular-nums ${hrs === 0 ? 'text-fog/40' : 'text-amber-400'}`}>
                          {hrs > 0 ? hrs.toFixed(1) : '—'}
                        </span>
                      </td>
                        ...
                        return (
                          ...
                          >
                            <div className="flex flex-col gap-1 h-full">
                              ...
                              {dayShifts.length === 0 && (
                                <span className="opacity-0 group-hover:opacity-40 transition-opacity text-amber-400 text-xs font-bold px-1 pt-1">
                                  + add
                                </span>
                              )}
                            </div>
        ...
        >
          {ctxMenu.shift ? (
            <>
              ...
              <CtxItem label="Remove Draft" onClick={handleCtxDelete} danger />
            </>
          ) : (
            <>
              <CtxItem
            ...
            <div className="flex items-center justify-between px-6 py-4 border-b border-rim/40">
              <div>
                <h2 className="font-heading font-bold text-ink text-lg">
                  {modal.mode === 'create' ? 'Add Draft Shift' : 'Edit Draft Shift'}
                </h2>
                <p className="text-amber-400 text-10 font-bold tracking-widest uppercase mt-0.5">Draft</p>
              </div>
            ...
            <div className="flex items-center justify-between px-6 py-4 border-t border-rim/40">
              ...
              <div className="flex gap-2">
                ...
                <button onClick={handleSave} disabled={saving} className="btn-primary">
                  {saving ? 'Saving…' : modal.mode === 'create' ? 'Add Draft' : 'Save Changes'}
                </button>
              </div>
            </div>
          </div>
...
function DraftShiftBlock({ shift, onClick, onDragStart, onDragEnd, onContextMenu, isCut }) {
  ...
  return (
    <button
      ...
      className="w-full text-left rounded px-2.5 py-1 leading-tight transition-all hover:brightness-110 cursor-grab active:cursor-grabbing"
      style={{
        backgroundColor: c.bg,
        color: c.text,
        border: `1px dashed ${c.text}40`,
      ...
      className={`w-full text-left px-4 py-1.5 text-sm transition-colors
        ${disabled   ? 'text-fog/40 cursor-not-allowed' :
          danger     ? 'text-red-400 hover:bg-red-500/10 cursor-pointer' :
                       'text-ink hover:bg-amber-500/10 cursor-pointer'}`}
    >
      {label}
    </button>
  );
